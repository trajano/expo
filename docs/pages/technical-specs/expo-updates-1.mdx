---
title: Expo Updates (version 1 draft)
sidebar_title: Expo Updates (version 1 draft)
---
...
Part order is not strict. A multipart response with no parts should be considered a no-op (no updates or directives available), though headers SHOULD be sent nevertheless.
